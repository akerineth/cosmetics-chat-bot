...
def get_strings_from_df(df):
    ...
    for index, row in df.iterrows():
        ...
        result += f"артикул{index + 1}: {row['Артикул']}\n"  # Артикул товара
        result += f"название{index + 1}: {row['Название']}\n"  # Название товара
        result += f"бренд{index + 1}: {row['Бренд']}\n"  # Бренд товара
        result += f"цена{index + 1}: {row['Цена']}\n"  # Цена товара
        result += f"описание{index + 1}: {row['Описание']}\n"  # Описание товара
        result += f"тип продукта{index + 1}: {row['Описание']}\n"  # Тип продукта (дублирует описание)
        result += "\n"  # Добавляем пустую строку как разделитель между записями
    return result  # Возвращаем итоговую строку

def top_recommendations(products, preferences, limitations, inverse_index, df):
    """
    ...
    required_columns = ['Артикул', 'Тип продукта', 'Название', 'Бренд', 'Цена', 'Описание']
